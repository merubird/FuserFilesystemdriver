...
//#define IOCTL_RESET_TIMEOUT       CTL_CODE(FILE_DEVICE_UNKNOWN, 0x80B, METHOD_BUFFERED,   FILE_ANY_ACCESS)  no longer exist
//#define IOCTL_SET_DEBUG_MODE      CTL_CODE(FILE_DEVICE_UNKNOWN, 0x801, METHOD_BUFFERED,   FILE_ANY_ACCESS)  no longer exist
//#define IOCTL_GET_ACCESS_TOKEN    CTL_CODE(FILE_DEVICE_UNKNOWN, 0x80C, METHOD_BUFFERED,   FILE_ANY_ACCESS)  no longer exist
...
typedef struct _EVENT_CONTEXT {  // TODO: revise
	ULONG	Length;
	ULONG	MountId;
	ULONG	SerialNumber;
	ULONG	ProcessId;
...
typedef struct _EVENT_INFORMATION {
	...
	union {
		...
		struct { // TODO: remove AccessToken
			HANDLE	Handle;
		} AccessToken;
	};
	ULONG64		Context;
